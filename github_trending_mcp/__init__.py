...
async def get_trending_repositories(limit: int, proxies: Optional[Dict[str, str]], ctx: Context) -> List[Dict[str, str]]:
    ...
    try:
        ...
        async with aiohttp.ClientSession() as session:
            ...
            async with session.get(url, headers=headers, proxy=aiohttp_proxy, timeout=aiohttp.ClientTimeout(total=10)) as response:
...
async def get_readme_content(repo_link: str, proxies: Optional[Dict[str, str]], ctx: Context) -> str:
    ...
    try:
        ...
        async with aiohttp.ClientSession() as session:
            ...
            async with session.get(repo_link, headers=headers, proxy=aiohttp_proxy, timeout=aiohttp.ClientTimeout(total=15)) as response:
                response.raise_for_status()  # 检查请求是否成功
                html_content = await response.text()
